 ...
 * message reception
 */
UA.prototype.receiveMessage = function(data) {
  var transaction;
  var message = Parser.parseMessage(data, this);

  if (!message) {
    return;
  }

  if (this.status === UA.C.STATUS_USER_CLOSED && message instanceof SIPMessage.IncomingRequest) {
    return;
  }

  // Do some sanity check
  if (!sanityCheck(message, this, this.transport)) {
    return;
  }
